...
def sber_text2embedding(model, tokenizer, device, text):
    ...
    if not text:
        return torch.empty((0, 1024)) # Return empty tensor for empty input
    ...
    try:
        ...
        with torch.no_grad():
            # Add tqdm wrapper around the dataloader for a progress bar
            for batch in tqdm(dataloader, desc="Encoding Batches"):
                ...
                all_embeddings.append(embeddings.cpu()) # Move to CPU immediately to save VRAM
